class API {

  static final String BASE_URL = "http://152.67.8.48/ActionAPI/AndroaidWebService.asmx/";

  static final String LOGIN = BASE_URL + "WsUtility_Emp_login";

  static final String ASSIGNED = BASE_URL + "upcoming_service_requests";

  static final String COMPLETED = BASE_URL + "completed_service_requests_dw";

  static final String CITIZEN_DETAIL = BASE_URL + "service_requests_detail";

  static final String UPDATE = BASE_URL + "update_service_request";

  static final String UPDATE_SUBMIT = BASE_URL + "sumbit_service_request";

   static final String Ws_Create_FDFM_CITY_DETAILS=BASE_URL+"Ws_Create_FDFM_CITY_DETAILS";

   static final String Ws_Create_FDFM_STAFF_SupportedBy=BASE_URL+"Ws_Create_FDFM_STAFF_SupportedBy";

   static final  String Ws_Create_FDFM_STAFF_CITY_COMPANY_NAME=BASE_URL+"Ws_Create_FDFM_STAFF_CITY_COMPANY_NAME";

   static final String Ws_Create_FDFM_New_v2 = BASE_URL+"Ws_Create_FDFM_New_v2";

  static final  String loginURL = BASE_URL+"Ws_Validate_Login?";

  static final  String downloadFDFM = BASE_URL+"Ws_FDM_ViewUser";

  static final  String customers = BASE_URL+"Ws_Secondry_Sale_Dynamic_Report_V2";

  static final  String target = BASE_URL+"Ws_Actual_Vs_Target_V2";

  static final  String notification = BASE_URL+"Ws_Notification";//

  static final  String order_itemmaster = BASE_URL+"Ws_Order_ItemMaster";

  static final String Ws_FDFM_SchemesDetails_V2 = BASE_URL+"Ws_FDFM_SchemesDetails_V2";

  static final String Ws_FollowUp_Detail_View_V2 = BASE_URL+"Ws_FollowUp_Detail_View_V2";

  static final String WS_Get_Syscode_Values = BASE_URL+"WS_Get_Syscode_Values";

  static final String WS_Get_Product_Type = BASE_URL+"WS_Get_Product_Type";

  static final String Ws_Get_SubArea = BASE_URL+"Ws_Get_SubArea";

  static final String Ws_Get_ItemCategory = BASE_URL+"Ws_Get_ItemCategory";

  static final String Ws_Save_Sales_Executive_Calls_Details = BASE_URL+"Ws_Save_Sales_Executive_Calls_Details";

  static final String Ws_Stock_Summary = BASE_URL+"Ws_Stock_Summary";

  static final String Ws_Sales_Return_View = BASE_URL+"Ws_Sales_Return_View";

  static final String Ws_FDFM_RetailerDetailsInfo_StaffWise_Details = BASE_URL+"Ws_FDFM_RetailerDetailsInfo_StaffWise_Details";

  static final String Ws_Complaint_Detail_View = BASE_URL+"Ws_Complaint_Detail_View";

  static final String Ws_Secondry_Sale_Dynamic_Report = BASE_URL+"Ws_Secondry_Sale_Dynamic_Report";

  static final String Ws_Events = BASE_URL+"Ws_Events";

  static final String Ws_Event_Summary = BASE_URL+"Ws_Event_Summary";

  static final String Ws_FDFM_ItemsModelNo = BASE_URL+"Ws_FDFM_ItemsModelNo";

  static final String WS_Get_Syscode_ValuesV2 = BASE_URL+"WS_Get_Syscode_ValuesV2";

  static final String Ws_Visit_Note_V3 = BASE_URL+"Ws_Visit_Note_V3";

  static final String Ws_Collection_Master = BASE_URL+"Ws_Collection_Master";

  static final String Ws_FDM_ItemMaster = BASE_URL+"Ws_FDM_ItemMaster";

  static final String Ws_Stock_Update = BASE_URL+"Ws_Stock_Update";

  static final String Ws_Upload_Picture_V1 = BASE_URL+"Ws_Upload_Picture_V1";

  static final String Ws_Competitor_Details = BASE_URL+"Ws_Competitor_Details";

  static final String Ws_Sales_Return = BASE_URL+"Ws_Sales_Return";

  static final String Ws_Special_Instruction = BASE_URL+"Ws_Special_Instruction";
}
